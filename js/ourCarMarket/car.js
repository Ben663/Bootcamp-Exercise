...
// way two
// const cars = agenciesArr.find((ag) => ag.agencyId === agencyId)?.cars ?? null;
// if (!cars) {
//     const res2 = [];
//     const cars2 = agenciesArr.map((ag) => ag.cars).flat();
//     console.log(cars2);
//     cars2.forEach((car) => {
//         for (const brand in car) {
//             res2.push(...car[brand]);
//         }
//     });
//     return res2;
...
const deleteCarFromAgency = (agencyObject, carNumber) => {
    let deletedCar;
    for (let key in agencyObject.cars) {
        const carArrLength = agencyObject.cars[key].length;
        agencyObject.cars[key] = agencyObject.cars[key].filter((car) => {
...
//decOrIncCashOfAgency(ourCarMarket.sellers[0], 1000000);
...
//? @param {string} - carNumber
//? @return {object[]} - allCarsOfCostumer
const deleteCarOfCostumer = (custumerObj, carNumber) => {
    let res;
    custumerObj.cars = custumerObj.cars.filter((car) => {
        if (car.carNumber === carNumber) {
            res = car;
        }
        return car.carNumber !== carNumber;
...
const decOrIncCashOfCustomer = (custumerObj, amount) => {
    if (custumerObj.cash + amount < 0) return;
    custumerObj.cash += amount;
    return custumerObj.cash;
};
//! ---------------- Hard ----------------------
